...
public class ResourceManager {
	...
	public void loadGameAudio() {
		try {
			...
			soundFall2 = SoundFactory.createSoundFromAsset(activity.getSoundManager(), activity, "fall2.ogg");
			soundJump2 = SoundFactory.createSoundFromAsset(activity.getSoundManager(), activity, "jump2.ogg");
    ...
    font = FontFactory.createStroke(activity.getFontManager(), activity.getTextureManager(), 512, 256, Typeface.create(Typeface.SANS_SERIF, Typeface.BOLD), 50, true, Color.WHITE_ABGR_PACKED_INT, 2, Color.BLACK_ABGR_PACKED_INT);
		//font = FontFactory.create(activity.getFontManager(), activity.getTextureManager(), 512, 512, Typeface.create(Typeface.SANS_SERIF, Typeface.NORMAL), 50);
        //font = FontFactory.create(activity.getFontManager(), activity.getTextureManager(), 256, 256, Typeface.create(Typeface.SANS_SERIF, Typeface.NORMAL), 32);
        font.prepareLetters("01234567890ABCDEFGHIJKLMNOPQRSTUVWXYZ.,!?".toCharArray());
//		font.prepareLetters("한글中文ひらがなカタカナ".toCharArray());
//		font.prepareLetters("देवनागरीอักษรไทย أَبْجَدِيَّة عَرَبِيَّة‎".toCharArray());
//		font.prepareLetters("01234567890ABCDEFGHIJKLMNOPQRSTUVWXYZÁÉ".toCharArray());

		font.load();
		itr = new TextureRegion(font.getTexture(), 0, 0, 256, 256);
	}
